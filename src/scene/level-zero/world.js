...
// Level 0 专用的地图元素：墙位收集、灯光、出口标志、氛围区域。
// BRIGHT_ZONES / DARK_ZONES 是 Level 0 教学/调度的固定区域。
...
export function collectWallTransforms() {
  ...
  const startCenter = cellCenter(1, 1);
  const exitCenter = cellCenter(cols - 2, rows - 2);
  fixtureCandidates.push(
    {
      x: startCenter.x,
      z: startCenter.z,
