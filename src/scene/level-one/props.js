...
import { CELL_SIZE, WALL_THICKNESS, CEILING_Y } from "../constants.js";
...
export function addLevelOneFloorZones(scene) {
  ...
  const origin = getLevelOneOrigin();
  ...
  const addTint = (zone, material, yOffset) => {
    ...
    mesh.rotation.x = -Math.PI / 2;
    mesh.position.set(
      origin.x + zone.col * CELL_SIZE + width / 2,
      yOffset,
      origin.z + zone.row * CELL_SIZE + height / 2,
    );
    scene.add(mesh);
  };
